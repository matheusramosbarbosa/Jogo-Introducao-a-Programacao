...
def character_importer(cols, rows, *path):
    ...
    new_dict = {}
    for row, direction in enumerate(('up', 'right', 'left', 'down')):
...
def checar_conexao(radius, entity, target, tolerance = 30):
    ...
    if relation.length() < radius:
        ...
        entity.facing_direction == 'up' and relation.x < 0 and abs(relation.y) < tolerance or \
        entity.facing_direction == 'down' and relation.x > 0 and abs(relation.y) < tolerance:
            return True